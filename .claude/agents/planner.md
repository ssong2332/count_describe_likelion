...
## 절차
0. 호출자가 준 아이디어 인터뷰 명세 요약(커버리지 11항목)을 입력으로 삼는다. 요약 없이 호출되면 임의로 요구사항을 지어내지 말고, 보고 첫 줄에 "인터뷰 명세 없이 호출됨 — 메인 세션의 아이디어 인터뷰 선행 필요"를 명시한 뒤 확인 가능한 범위만 기록한다
1. 인터뷰 명세를 기능 요구사항(F-xx)·비기능 요구사항(N-xx)으로 분해해 PRD.md에 기록 — 승인 기준은 검증 가능한 조건으로 쓴다 (Given-When-Then 권장)
...
3. 판단이 갈리는 지점과 인터뷰에서 "미정"으로 남은 항목은 전부 Open Questions 표에 추가
