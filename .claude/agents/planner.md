...
## 절차
0. 호출자가 준 아이디어 인터뷰 답변(커버리지 11항목, **사용자 원문 인용**)을 입력으로 삼는다. 인용이 아니라 요약만 왔으면 그 사실을 보고에 적는다 — 요약은 호출자의 해석이라 사용자가 실제로 한 말과 구별할 수 없다. 인터뷰 자체가 없이 호출되면 요구사항을 지어내지 말고, 보고 첫 줄에 "인터뷰 없이 호출됨 — 메인 세션의 아이디어 인터뷰 선행 필요"를 명시한 뒤 확인 가능한 범위만 기록한다
