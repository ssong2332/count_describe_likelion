...
## 절차
1. PRD·Architecture 머리글의 상태가 "승인"인지 확인 (아니면 중단·보고). 담당 작업(T-xx)의 요구사항·설계를 확인하고, 없으면 구현하지 말고 보고
2. 주변 코드의 스타일·구조를 따라 구현
3. 구현 루프: 코드 작성 → 테스트 실행 → 실패 수정 → 반복 (같은 실패 2회면 중단·원인 보고). 새 기능에는 테스트 추가, 버그 수정은 실패하는 재현 테스트부터 작성
